...
impl Player for MarineThorRushAI {
	...
	fn get_player_settings(&self) -> PlayerSettings {
		PlayerSettings::new(Race::Terran).with_name("Moe Betta")
	}
}

impl MarineThorRushAI {
	...
	fn build(&mut self) {
		...
		let proxy = self.start_location.towards(self.game_info.map_center, 100.0);
			...
			&& self.can_afford(UnitTypeId::SupplyDepot, false)
		{
			if let Some(location) =
				self.find_placement(UnitTypeId::SupplyDepot, main_base, PlacementOptions { addon: true, ..Default::default()})
			{
		...
		if self.counter().all().count(UnitTypeId::Barracks) < 2
        ...
        if self.counter().all().count(UnitTypeId::Barracks) > 1
			&& self.counter().all().count(UnitTypeId::Factory) < 3
		...
		{
			for factory in self.units.my.structures.iter().of_type(UnitTypeId::Factory).ready().idle() {
				factory.use_ability(BuildTechLabFactory, false);
			}
		}
		/*
		 if self.counter().all().count(UnitTypeId::Starport) < 1
            && self.can_afford(UnitTypeId::Starport, false)
        {   
            if let Some(location) = self.find_placement(
			UnitTypeId::Starport,
			main_base,
			PlacementOptions {
				...
				if let Some(builder) = self.get_builder(location, &mineral_tags) {
					builder.build(UnitTypeId::Starport, location, false);
					self.subtract_resources(UnitTypeId::Starport, false);
				}
            }
        }
		if self.counter().all().count(UnitTypeId::Starport) >= 1
			&& self.can_afford(UnitTypeId::Reactor, false) 
		{
			for starport in self.units.my.structures.iter().of_type(UnitTypeId::Starport).ready().idle() {
				starport.use_ability(BuildReactorStarport, false);
			}
		}
		*/
	...
	fn train(&mut self) {
		...
		if self.can_afford(UnitTypeId::Thor, true) {
			if let Some(factory) = self
				...
				.structures
				.iter()
				.find(|u| u.type_id() == UnitTypeId::Factory && u.is_ready() && u.is_almost_idle())
			{
...
*/
	...
	fn execute_micro(&mut self) {
		...
		// Marine micro
		let marine = self.units.my.units.of_type(UnitTypeId::Marine);
		if marine.is_empty() {
			return;
		}
...
fn main() -> SC2Result<()> {
	...
	let mut bot = MarineThorRushAI::default();
	run_vs_computer(
        	&mut bot,
