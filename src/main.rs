...
impl MarineThorRushAI {
        ...
        *last_loop = game_loop;
        ...
        let minerals_near_base =
            if idle_workers.len() > deficit_minings.len() + deficit_geysers.len() {
                let minerals =
                    mineral_fields.filter(|m| bases.iter().any(|base| base.is_closer(11.0, *m)));
    ...
    fn build(&mut self) {
        ...
        if self.counter().all().count(UnitTypeId::Barracks) < 3
    ...
    fn train(&mut self) {
        ...
        if self.can_afford(UnitTypeId::Marine, true) {
            if let Some(barracks) =
                self.units.my.structures.iter().find(|u| {
                    u.type_id() == UnitTypeId::Barracks && u.is_ready() && u.is_almost_idle()
                })
            {
                    ...
                    if is_retreating {
                            ...
                            .closest(u)
                        {
                            Some(closest_attacker) => {
                                ...
                                u.attack(Target::Tag(closest_attacker.tag()), false);
                            }
                            None => {
